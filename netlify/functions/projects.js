...
export default async function handler(req) {
  ...
  if (req.method === "POST") {
    ...
    if (action === "update_project") {
      ...
      // Project-wide cable-length multipliers — a human judgment call on route
      // quality, applied per device_instance at report render time based on
      // which tier actually routed it (device_instances.routed_via_tier3):
      // default_length_multiplier for Tier 3 (wall-aware) runs, fallback_
      // length_multiplier for Tier 1 (straight-line) runs. Each is one value
      // for the whole project — does not vary by page. Editable from the
      // confidence map or directly on the report.
      ...
      if (fallback_length_multiplier !== undefined) {
        const m = Number(fallback_length_multiplier);
        if (!Number.isFinite(m) || m <= 0) return err("fallback_length_multiplier must be a positive number");
        patch.fallback_length_multiplier = m;
      }
