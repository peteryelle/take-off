...
-- confirm: 8 new tables, all RLS on
SELECT c.relname, c.relrowsecurity AS rls_on
FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
WHERE n.nspname = 'takeoff'
  AND c.relname IN ('page_regions','discovery_sessions','discovery_clusters','discovery_results',
                    'wall_calibrations','wall_calibration_runs','page_wall_geometry','waypoints')
ORDER BY 1;
