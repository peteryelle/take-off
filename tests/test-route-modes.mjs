...
eq('defaults per mode', DEFAULT_MULTIPLIERS, { straight: 1, right_angle: 1, routed: 1.35 });
...
// fallbacks
eq('missing mode multiplier -> default', effectiveRouting({ route_mode: 'routed' }, {}), { mode: 'routed', multiplier: 1.35 });
eq('bad values fall back', effectiveRouting({ route_mode: 'diagonal', straight_multiplier: -2 }, { route_multiplier: 0 }), { mode: 'straight', multiplier: 1 });
...
// Routed at its default multiplier must equal the old pass-batch formula exactly:
//   run = parseFloat((dist * 1.35 / ptsPerFt).toFixed(1)); total = parseFloat((run + stub).toFixed(1))
let mismatches = 0, seed = 7;
const rnd = () => (seed = (seed * 16807) % 2147483647) / 2147483647;
const routedDefault = effectiveRouting({ route_mode: 'routed' }, {}).multiplier;
for (let i = 0; i < 5000; i++) {
  ...
  const L = lengthsFt(dist, ppf, routedDefault, stub);
  if (L.run_ft !== oldRun || L.route_ft !== oldTotal) mismatches++;
}
eq('routed default == old formula (5000 random cases)', mismatches, 0);
